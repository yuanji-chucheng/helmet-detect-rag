...
class Results(SimpleClass, DataExportMixin):
    ...
    该类提供了处理来自各种Ultralytics模型推理结果的全面功能，包括检测、分割、分类和姿态估计。它支持可视化、数据导出和各种坐标变换。

    属性：
    - orig_img (np.ndarray): 原始图像，以numpy数组形式存储。
    ...
    方法：
    - update: 使用新的检测数据更新Results对象。
    - cpu: 返回Results对象的副本，其中所有张量已移动到CPU内存。
    - numpy: 将Results对象中的所有张量转换为numpy数组。
    - cuda: 将Results对象中的所有张量移动到GPU内存。
    - to: 将所有张量移动到指定的设备和数据类型。
    - new: 创建一个新的Results对象，具有相同的图像、路径、名称和速度属性。
    - plot: 在输入RGB图像上绘制检测结果。
    - show: 显示带有标注推理结果的图像。
    - save: 将标注后的推理结果图像保存到文件。
    - verbose: 为结果中的每个任务返回日志字符串。
    - save_txt: 将检测结果保存到文本文件。
    - save_crop: 将裁剪的检测图像保存到指定目录。
    - summary: 将推理结果转换为汇总字典。
    - to_df: 将检测结果转换为Polars数据框。
    - to_json: 将检测结果转换为JSON格式。
    - to_csv: 将检测结果转换为CSV格式。
    ...
    def plot(
        ...
        - line_width (float | None): 边界框的线宽。如果为None，则根据图像尺寸自动缩放。
        - font_size (float | None): 文本的字体大小。如果为None，则根据图像尺寸自动缩放。
        - font (str): 文本使用的字体。
        - pil (bool): 是否将图像作为PIL图像返回。
        - img (np.ndarray | None): 要绘制结果的图像。如果为None，则使用原始图像。
        - im_gpu (torch.Tensor | None): 用于加速掩码绘制的GPU上的归一化图像。
        - kpt_radius (int): 绘制关键点的半径。
        - kpt_line (bool): 是否绘制连接关键点的线条。
        - labels (bool): 是否绘制边界框的标签。
        - boxes (bool): 是否绘制边界框。
        - masks (bool): 是否绘制掩码。
        - probs (bool): 是否绘制分类概率。
        - show (bool): 是否显示标注后的图像。
        - save (bool): 是否保存标注后的图像。
        - filename (str | None): 如果save为True，保存图像的文件名。
        - color_mode (str): 指定颜色模式，例如'instance'或'class'。
        - txt_color (tuple[int, int, int]): 为分类任务指定RGB文本颜色。

        返回值：
        - (np.ndarray): 标注后的图像作为numpy数组。
...
class Boxes(BaseTensor):
    ...
    该类提供了处理检测框的全面功能，包括坐标、置信度分数、类别标签和可选的追踪ID。它支持多种框格式，并提供在不同坐标系之间进行简便操作和转换的方法。

    属性：
    data (torch.Tensor | np.ndarray): 包含检测框及相关数据的原始张量。
    orig_shape (tuple[int, int]): 原始图像尺寸（高度，宽度）。
    is_track (bool): 指示框数据中是否包含追踪ID。
    ...
    id (torch.Tensor | None): 每个边界框的追踪ID（如果可用）。
    xywh (torch.Tensor | np.ndarray): [x, y, 宽度, 高度] 格式的边界框。
    xyxyn (torch.Tensor | np.ndarray): 相对于orig_shape的归一化[x1, y1, x2, y2]格式边界框。
    xywhn (torch.Tensor | np.ndarray): 相对于orig_shape的归一化[x, y, 宽度, 高度]格式边界框。
    方法：
    cpu: 返回对象副本，其中所有张量位于CPU内存中。
    numpy: 返回对象副本，其中所有张量转换为numpy数组。
    cuda: 返回对象副本，其中所有张量位于GPU内存中。
    to: 返回对象副本，其中张量位于指定设备和数据类型上。
